...
import { PengawasanIinNasionalApplication, PaymentDocument } from "@/types";
import { router } from "@inertiajs/react";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { AlertCircle, Clock, CreditCard, Download, FileText, Upload } from "lucide-react";
import { useState } from "react";

export default function PaymentTab({ application }: { application: PengawasanIinNasionalApplication }) {
    const [uploading, setUploading] = useState(false);
    const [uploadProgress, setUploadProgress] = useState(0);
    const [formData, setFormData] = useState({
        payment_proof_documents: [] as File[],
    });

    const uploadPaymentProof = (e: React.FormEvent) => {
        e.preventDefault();
        if (formData.payment_proof_documents.length === 0) {
        ...
        formData.payment_proof_documents.forEach((file, index) => {
            data.append(`payment_proof_documents[${index}]`, file);
        });
        ...
        router.post(route('pengawasan-iin-nasional.upload-payment-proof', application.id), data, {
            onProgress: (progress) => {
            ...
            onSuccess: () => {
                setUploading(false);
                setUploadProgress(0);
                setFormData({ payment_proof_documents: [] });
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            setFormData({
                ...formData,
                payment_proof_documents: [...formData.payment_proof_documents, ...filesArray],
            });
        }
    };

    const removeFile = (index: number) => {
        const updatedFiles = formData.payment_proof_documents.filter((_, i) => i !== index);
        setFormData({
            ...formData,
            payment_proof_documents: updatedFiles,
        });
    };
            ...
            <CardContent className="space-y-6">
                                            ...
                                            {application.payment_documents.map((document: PaymentDocument, index: number) => (
                                                ...
                                                >
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() =>
                                                            window.open(
                                                                route('pengawasan-iin-nasional.download-file', [application.id, 'payment_document', 0]),
                                                                '_blank',
                                                            )
                                                        }
                                ...
                                {application.payment_proof_documents && application.payment_proof_documents.length > 0 ? (
                                        ...
                                        {application.payment_proof_documents.map((document: PaymentDocument, index: number) => (
                                            ...
                                            >
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() =>
                                                        window.open(
                                                            route('pengawasan-iin-nasional.download-file', [application.id, 'payment_proof', 0]),
                                                            '_blank',
                                                        )
                                                    }
                                                >
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <Button
                                            ...
                                            onClick={() =>
                                                window.open(
                                                    route('pengawasan-iin-nasional.download-file', [application.id, 'payment_proof']),
                                                    '_blank',
                                                )
                                            }
                                        >
                ...
                {application.status === 'pembayaran' ? (
                    <form onSubmit={uploadPaymentProof} className="space-y-4">
                        ...
                        <Button
                            type="submit"
                            disabled={uploading || formData.payment_proof_documents.length === 0}
