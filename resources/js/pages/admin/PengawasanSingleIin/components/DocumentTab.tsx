...
import { PaymentDocument, PengawasanSingleIinApplication } from "@/types";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { Award, CreditCard, Download, FileText, Shield } from "lucide-react";
import { useState } from "react";

export default function DocumentTab({ application }: { application: PengawasanSingleIinApplication }) {

    const [isReimModalOpen, setIsReimModalOpen] = useState(false);

    const canDownloadCertificate = application.status === 'terbit';

    const downloadFile = (type: string, index?: number) => {
        window.open(
            route('pengawasan-single-iin.download-file', {
                    ...
                    {application.status === 'terbit' && (
                        <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                            <div className="flex gap-3 items-center">
                                ...
                                <div>
                                    <p className="font-medium text-gray-800">Dokumen Pemantauan Single IIN</p>
                                    <p className="text-sm text-gray-500">Dokumen Pemantauan Single IIN</p>
                                </div>
                            </div>
                            {canDownloadCertificate && (
                                <Button variant="outline" size="sm" onClick={() => downloadFile('certificate', 0)}>
