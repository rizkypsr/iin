...
export default function AdminIinNasionalShow({ auth, application, statusLogs, application_counts }: Props) {
    ...
    const handleCompleteFieldVerification = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formData.append('complete_verification', '1');
            formData.append('notes', verificationNotes || 'Verifikasi lapangan selesai dan IIN diterbitkan');

            router.post(
                route('admin.iin-nasional.upload-field-verification-documents', application.id),
                formData,
                {
                    forceFormData: true,
                    onSuccess: () => {
    ...
    const handleIssueIIN = async () => {
        ...
        try {
            ...
            router.post(
                route('iin-nasional.issue-iin', application.id),
                formData,
                {
                    onSuccess: () => {
    ...
    const handleUploadCertificate = async () => {
        ...
        try {
            ...
            router.post(
                route('iin-nasional.upload-certificate', application.id),
                formData,
                {
                    onSuccess: () => {
                            ...
                            <Dialog>
                                ...
                                <DialogContent className="max-w-md">
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>
                                            Upload sertifikat IIN untuk menyelesaikan verifikasi lapangan. Status aplikasi akan langsung berubah menjadi <strong>terbit</strong>.
                                        </DialogDescription>
                                    </DialogHeader>
                                            ...
                                            <div key={log.id} className="p-4 transition-colors hover:bg-gray-50">
                                                <div className="flex items-start gap-4">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        {log.admin && (
                                                            <p className="text-sm text-gray-600">oleh {log.admin.name}</p>
                                                        )}
                                                        {log.notes && (
